...
class IrSourceCompilerForInline(
    ...
    override fun doCreateMethodNodeFromSource(
        ...
        val irFunction = ((callElement as IrCall).symbol.owner as IrFunction).let { irFunction ->
            if (!callDefault || irFunction.parent !is IrFile) irFunction
            else {
                val irFile = irFunction.parent as IrFile
                irFile.declarations.filterIsInstance<IrClass>().single {
                    //find class for package part
                    it.thisReceiver == null
                }.declarations.filterIsInstance<IrFunction>().single {
